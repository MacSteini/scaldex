...
class RootRunnerTests(unittest.TestCase):
    ...
    def test_api_key_is_not_written_to_generated_files(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            output = io.StringIO()
            with Cwd(root), redirect_stdout(output), patch.dict("os.environ", {"CODEX_API_KEY": "sk-test-secret"}, clear=True), patch.object(module, "create_fixture"), patch.object(module, "run_benchmark", side_effect=fake_run_benchmark):
