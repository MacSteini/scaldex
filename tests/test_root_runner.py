...
class RootRunnerTests(unittest.TestCase):
    def test_help_text_explains_common_enduser_flow(self) -> None:
        ...
        self.assertIn("For Codex-assisted follow-up, provide scaldex-run/CODEX_HANDOFF.md and the measured subject/ contents", help_text)
    ...
    def test_non_scaldex_run_dir_error_does_not_print_absolute_path(self) -> None:
        module = load_root_runner()
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            ...
            run_dir.mkdir()
            (run_dir / "note.txt").write_text("not generated by scaldex\n", encoding="utf-8")
            with Cwd(root), redirect_stdout(io.StringIO()), redirect_stderr(io.StringIO()), patch.dict("os.environ", {"CODEX_API_KEY": "sk-test"}, clear=True):
                with self.assertRaises(SystemExit) as ctx:
                    module.main(["--model", "model", "--run-dir", str(run_dir)])
        self.assertIn("Refusing to replace non-scaldex --run-dir", str(ctx.exception))
        self.assertIn("existing-run", str(ctx.exception))
    ...
    def test_print_result_replays_existing_result_without_subject_or_api_key(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertIn("- Provide with: the measured subject/ contents and a clear task.", output.getvalue())
    ...
    def test_api_key_is_not_written_to_generated_files(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            subject.mkdir()
            (subject / "AGENTS.md").write_text("# Agents\n", encoding="utf-8")
            (root / "scaldex-run").mkdir()
            (root / "scaldex-run" / module.GENERATED_MARKER).write_text("generated by scaldex\n", encoding="utf-8")
            ...
            result_payload = json.loads((root / "scaldex-run" / "result.json").read_text(encoding="utf-8"))
            self.assertEqual(result_payload["run_config"]["subject_dir"], "subject")
            self.assertEqual(result_payload["artifacts"]["result_json"], "result.json")
            self.assertEqual(result_payload["artifacts"]["raw_dir"], "scaldex-run/raw")
            self.assertNotIn(str(root), json.dumps(result_payload))
    ...
    def test_refuses_default_unmarked_nonempty_run_dir(self) -> None:
        module = load_root_runner()
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            subject = root / "subject"
            subject.mkdir()
            (subject / "AGENTS.md").write_text("# Agents\n", encoding="utf-8")
            default_run = root / "scaldex-run"
            default_run.mkdir()
            (default_run / "user-file.txt").write_text("do not delete\n", encoding="utf-8")
            with Cwd(root), redirect_stdout(io.StringIO()), redirect_stderr(io.StringIO()), patch.dict("os.environ", {"CODEX_API_KEY": "sk-test-secret"}, clear=True):
                with self.assertRaises(SystemExit) as ctx:
                    module.main(["--model", "model"])
            self.assertIn("Refusing to replace non-scaldex --run-dir", str(ctx.exception))
            self.assertTrue((default_run / "user-file.txt").exists())

    def test_subject_symlink_error_is_reported_without_traceback(self) -> None:
        module = load_root_runner()
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
    ...
    def test_archives_previous_compact_report_before_replacing_run_dir(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertEqual(payload["source_run_dir"], "scaldex-run")
