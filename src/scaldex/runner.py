...
def prepare_generated_dir(path: Path, *, allow_unmarked_existing: bool = False) -> None:
    if path.exists():
        ...
        if has_content and not marker.exists() and not allow_unmarked_existing:
            raise ValueError(f"Refusing to replace non-scaldex output directory without {GENERATED_MARKER}: {path}")
        shutil.rmtree(path)
    path.mkdir(parents=True, exist_ok=True)
    (path / GENERATED_MARKER).write_text("generated by scaldex\n", encoding="utf-8")
...
def init_git_snapshot(workdir: Path) -> None:
    ...
    subprocess.run(
        ["git", "-c", "user.name=scaldex", "-c", "user.email=scaldex@example.invalid", "commit", "-q", "-m", "run snapshot"],
        cwd=workdir,
        check=True,
    )
...
def run_one(
    *,
    ...
    if workspace_root is None:
        workspace_parent = Path(tempfile.mkdtemp(prefix=f"scaldex-{run_id}-"))
    else:
